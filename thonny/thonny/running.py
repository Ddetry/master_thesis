...
from thonny.assistance import SubprocessProgramAnalyzer
...
from thonny.debugger import AssistantView
...
class Runner:
    ...
    def _init_commands(self) -> None:
        ...
        get_workbench().add_command(
            ...
            "MyPy checker",
            caption=RUN_COMMAND_CAPTION,
            handler=self.cmd_run_mypy_checker,
            group=10,
            image="mypy",
    ...
    # modif
    def cmd_run_mypy_checker(self) -> bool:
        filename = get_saved_current_script_filename()
        result = subprocess.run(['mypy', str(filename)], stdout=subprocess.PIPE)

        res = str(result.stdout)
        res.replace("\n", " ")
        res = res[2:len(res)-3]

        get_shell().text._insert_text_directly(res, ("io", "stderr"))
        get_shell().text._insert_prompt()
