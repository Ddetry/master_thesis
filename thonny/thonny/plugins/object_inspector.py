...
class ObjectInspector(ttk.Frame):
    ...
    def set_object_info(self, object_info):
        ...
        if object_info is None or "error" in object_info:
            ...
            # modif
            if(object_info["full_type_name"] == "list" or object_info["full_type_name"] == "set" or object_info["full_type_name"] == "tuple" or object_info["full_type_name"] == "dict"):
                addr = thonny.memory.format_object_id(object_info["id"])
                repr = self.parcour_var(object_info["id"])
                value = object_info["repr"].replace('<', "'").replace(">","'")
                myval = self.correct_object_addr(eval(value))
                self.send_to_GraphViz(object_info, myval)

                if repr != None:
                    get_workbench().event_generate("Insert_line", parent_id='', item=myval, item_name=repr[0], depth=0,max_depth=100)
                else:
                    get_workbench().event_generate("Insert_line", parent_id='', item=myval, item_name=addr, depth=0,max_depth=100)

            # si c'est un objet
            elif "__main__" in object_info["full_type_name"]:
                item = self.from_attributes_to_tab(object_info["attributes"])
                self.send_to_GraphViz(object_info, item)
                get_workbench().event_generate("Insert_object", parent_id='', item=item, item_name=object_info["repr"], depth=0, max_depth=100)


            elif(object_info["full_type_name"] == "int" or object_info["full_type_name"] == "float" or object_info["full_type_name"] == "str" or object_info["full_type_name"] == "bool"):
                addr = thonny.memory.format_object_id(object_info["id"])
                repr = self.parcour_var(object_info["id"])
                if repr != None:
                    get_workbench().event_generate("Insert_line", parent_id='', item=object_info["repr"], item_name=repr[0], depth=0,max_depth=100)
                else:
                    get_workbench().event_generate("Insert_line", parent_id='', item=object_info["repr"], item_name=addr, depth=0,max_depth=100)
    ...
    # mytab est la representation correcte des elements
    # pour les objets, mytab est le tableau des attributs
    def send_to_GraphViz(self, info, mytab):
        if info["full_type_name"] == "list" or info["full_type_name"] == "set" or info["full_type_name"] == "tuple":
            get_workbench().event_generate("Open_GraphViz_with_tab",element=mytab, id= info["id"], name = self.parcour_var(info["id"]))

        elif info["full_type_name"] == "dict":
            mydict = self.from_entries_to_tab(info["entries"])
            get_workbench().event_generate("Open_GraphViz_with_dict",entries=mydict, id= info["id"], name = self.parcour_var(info["id"]))

        else: # c'est un objet
            get_workbench().event_generate("Open_GraphViz_with_object", attributes=mytab, id= info["id"], name=self.parcour_var(info["id"]))
    ...
    def correct_object_addr(self, mytab):
        newtab = []
        for elem in mytab:
            if type(elem) == str and "__main__" in elem:
                newelem = elem[-11:]
                newtab.append(newelem[0:11])
            elif type(elem) == list or type(elem) == set or type(elem) == tuple:
                newtab.append(self.correct_object_addr(elem))
            else:
                newtab.append(elem)
        return newtab


    def from_attributes_to_tab(self, attributes):
        myattributes = []
        for ref, val in attributes.items():
            if type(val[1]) is str:
                if "__main__" in val[1]:
                    elem = val[1]
                    newelem = elem[-12:]
                    myattributes.append((ref,newelem[0:11]))
                else:
                    myattributes.append((ref,val[1]))
            else:
                myval = eval(val[1])
                myattributes.append((ref, myval))
        return myattributes


    def from_element_to_tab(self, elements):
        myelements = []
        for elem in elements:  # elem is a ValueInfo tuple
            if "__main__" in elem[1]:
                myelements.append(str(elem[1]))
            else:
                myelements.append(eval(elem[1]))
        return myelements

    def from_entries_to_tab(self, entries):
        mydict = {}
        for elem in entries:
            if "__main__" in elem[1][1]:
                val = elem[1][1]
                newelem = val[-12:]
                mydict.update({elem[0][1] : str(newelem[0:11])})
            else:
                mydict.update({elem[0][1] : eval(elem[1][1])})
        return mydict
